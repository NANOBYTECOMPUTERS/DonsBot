...
class Capture(threading.Thread):
    def __init__(self):
        ...
        if not (isinstance(cfg.detection_window_width, (int, float)) and cfg.detection_window_width > 0):
            raise ValueError("Invalid detection_window_width in config")
        if not (isinstance(cfg.detection_window_height, (int, float)) and cfg.detection_window_height > 0):
            raise ValueError("Invalid detection_window_height in config")
        if not (isinstance(cfg.bettercam_capture_fps, (int, float)) and cfg.bettercam_capture_fps > 0):
            cfg.bettercam_capture_fps = 1  # Fallback FPS
    ...
    def print_startup_messages(self):
        ...
        try:
            ...
            with open(version_path, 'r') as f:
                line = f.readline()
                # Optionally check if the line has the expected format before splitting
                if '=' in line:
